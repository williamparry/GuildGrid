    ...
    try {
        ...
        if (!data) {
            const { error: insertError } = await supabase
                ...
                .insert([{ id: guildId }]);
            if (insertError) {
                console.error('Error adding new guild to database:', insertError);
            } else {
...
async function updateGuildMemberships(guild) {
    try {
        ...
        const storedMemberIds = storedMemberships.map(membership => membership.user_id);
        const membersToAdd = currentMemberIds.filter(id => !storedMemberIds.includes(id));
        const membersToRemove = storedMemberIds.filter(id => !currentMemberIds.includes(id));
        // Perform bulk insert for new members
        if (membersToAdd.length > 0) {
            const newMembers = membersToAdd.map(userId => ({ user_id: userId, guild_id: guild.id }));
...
const startBot = async () => {
    ...
    client.on('guildMemberAdd', async member => {
        ...
        const { error } = await supabase
            .from('gg_guild_memberships')
            .insert([
                { user_id: member.user.id, guild_id: member.guild.id }
            ]);
    ...
    client.on('interactionCreate', async interaction => {
        ...
        if (interaction.commandName === 'grid') {
            ...
            const gridName = interaction.options.getString('grid_name') || `${interaction.user.username}'s grid`;
            const randomString = Math.random().toString(36).substr(2, 5); // Short random string
            const gridSlugBase = gridName.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
            ...
            const gridUrl = `https://guildgrid.gg/grids/${gridSlug}`;
